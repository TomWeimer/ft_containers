...
class MapAt : public TestBase
{
	...
	public:
		...
		MapAt();
		MapAt(const MapAt& origin);
		MapAt& operator=(const MapAt& origin);
		~MapAt();
	private:
		template<class U>
		void execute_test();
...
MapAt::MapAt(const MapAt& origin)
	: TestBase(origin) {}
...
MapAt& MapAt::operator=(const MapAt& origin)
{
	if (this == &origin)
		return (*this);
...
std::stringstream& MapAt::testInstruction(int namespaces)
{
	result.str("");
...
void MapAt::execute_test()
{
	...
	this->timer.start();
	result << "return value: " << filled_map.at(get_single<key_type>()) << std::endl;
	this->timer.stop();
...
template<class U>
std::string MapAt::show_map_content(U map_type)
{
	typedef U container_type;
