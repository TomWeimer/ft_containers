...
class VectorEmpty : public TestBase
{
	private:
	...
	public:
		...
		VectorEmpty();
		VectorEmpty(const VectorEmpty& origin);
		VectorEmpty& operator=(const VectorEmpty& origin);
		~VectorEmpty();
	private:
		template<class U>
		void execute_test();
...
VectorEmpty::VectorEmpty(const VectorEmpty& origin)
	: TestBase(origin) {}
...
VectorEmpty& VectorEmpty::operator=(const VectorEmpty& origin)
{
	if (this == &origin)
		return (*this);
...
std::stringstream& VectorEmpty::testInstruction(int namespaces)
{
	result.str("");
...
void VectorEmpty::execute_test()
{
	...
	this->timer.start();
	result << "empty: " << container.empty() << std::endl;
	this->timer.stop();
...
template<class U>
std::string VectorEmpty::show_vector_content(U vector_type)
{
	typedef U container_type;
