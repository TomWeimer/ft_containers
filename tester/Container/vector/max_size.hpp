#ifndef VECTOR_MAX_SIZE_HPP
#define VECTOR_MAX_SIZE_HPP

#include "../../TestTemplate/TestBase.hpp"

class VectorMaxSize : public TestBase
{
	private:
	...
	public:
		...
		VectorMaxSize();
		VectorMaxSize(const VectorMaxSize& origin);
		VectorMaxSize& operator=(const VectorMaxSize& origin);
		~VectorMaxSize();
	private:
		template<class U>
		void execute_test();
...
VectorMaxSize::VectorMaxSize()
	: TestBase("max_size.log"){}


VectorMaxSize::VectorMaxSize(const VectorMaxSize& origin)
	: TestBase(origin) {}
...
VectorMaxSize& VectorMaxSize::operator=(const VectorMaxSize& origin)
{
	if (this == &origin)
		return (*this);
...
std::stringstream& VectorMaxSize::testInstruction(int namespaces)
{
	result.str("");
...
template<class U>
void VectorMaxSize::execute_test()
{
	...
	this->timer.start();
	result << "max size: " << container.max_size() << std::endl;
	this->timer.stop();
...
template<class U>
std::string VectorMaxSize::show_vector_content(U vector_type)
{
	typedef U container_type;
