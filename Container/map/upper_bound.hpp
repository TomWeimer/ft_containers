#ifndef MAP_UPPER_BOUND_HPP
#define MAP_UPPER_BOUND_HPP
#include "../../TestTemplate/TestBase.hpp"

class MapUpperBound : public TestBase
{
	private:
	...
	public:
		...
		MapUpperBound();
		MapUpperBound(const MapUpperBound& origin);
		MapUpperBound& operator=(const MapUpperBound& origin);
		~MapUpperBound();
	private:
		template<class U>
		void execute_test();
...
MapUpperBound::MapUpperBound()
	: TestBase("upper_bound.log"){}


MapUpperBound::MapUpperBound(const MapUpperBound& origin)
	: TestBase(origin) {}
...
MapUpperBound& MapUpperBound::operator=(const MapUpperBound& origin)
{
	if (this == &origin)
		return (*this);
...
std::stringstream& MapUpperBound::testInstruction(int namespaces)
{
	result.str("");
...
template<class U>
void MapUpperBound::execute_test()
{
	...
	this->timer.start();
	iterator tmp = filled_map.upper_bound(*get_multiple<key_type>());
	this->timer.stop();
...
template<class U>
std::string MapUpperBound::show_map_content(U map_type)
{
	typedef U container_type;
	typedef typename container_type::iterator iterator;
